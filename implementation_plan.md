...
- **Confirmed load-bearing bug, found and fixed during Phase 3's first real factory run:** `ctx.agent(prompt, { agent: "<named-agent>" })`'s `agent` option — the mechanism §4's design assumed would apply each `.github/agents/*.md`'s specialized system prompt per fan-out stage — is documented in this SDK build's generated types as "accepted but not yet honored." Every subagent actually runs as a generic agent; the named `.github/agents/*.md` files are **not** consulted by the runtime today. Because several fan-out stages originally shared one generic prompt template and one generic `{title, content, citations}` schema, the model had no way to tell which of five different sections it was writing, and produced near-duplicate content across them — a real, observed failure mode, not a hypothetical one. The fix (now in `.github/extensions/artha-tools/extension.mjs`): embed each section's full task-specific instructions (condensed from its `.md` file) directly in that stage's prompt text, not merely implied by the (currently inert) `agent` option. Keep `agent: job.agent` in the call for forward compatibility, but never rely on it alone. Re-check this note once the SDK changelog says `agent` is honored — the inline-instructions duplication could then be reduced back to a name reference.
- **First real, validated, factory-written dossier landed:** `dossiers/Eco Recyc/2cbc27d8-3623-4e1b-b1b3-77699bd6164d.md`, produced by an actual `run_factory("artha-dossier", ...)` call (not a manually-orchestrated session) — 8 framework-section subagents fanned out, 8 adversarial citation-verification subagents, 1 narrative-assembly subagent, then the deterministic `validate_dossier` gate and `write-dossier` CLI, exactly 17 subagents total, no wasted duplicate work. Real cost: **~300 AI credits** for one Track A dossier — the factory's own declared default of `maxAiCredits: 5` (matching `config/artha.example.toml`'s `[budget] max_ai_credits_per_dossier = 5.0`) is off by roughly 60x and must be revised before any non-experimental use; a per-invocation override (or a config change) is required until then. The dossier's own validation legitimately failed the §15 Moat & Understandability gate on genuine evidence gaps (not a bug) — `write_dossier`'s "write even on failure, stage=rejected" behavior worked exactly as designed.
